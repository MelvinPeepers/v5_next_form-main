...
module.exports = { 
  async onError(meta) { 
    const fileStream = createReadStream(join(meta.constants.FUNCTIONS_DIST, '___netlify-server-handler.zip'), { 
      highWaterMark: 149 * 1024 * 1024 
    }) 
    console.log(statSync(join(meta.constants.FUNCTIONS_DIST, '___netlify-server-handler.zip')).size / (1024 ^ 2)) 
    const headers = { 
    ...
    const abc = await fetch('https://content.dropboxapi.com/2/files/upload_session/finish', { 
      headers: { 
        ...headers, 
        'dropbox-api-arg': JSON.stringify({ 
          commit: { 
